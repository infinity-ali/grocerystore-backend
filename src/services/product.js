...
export default class productService {
    ...
    async AddProduct(input) {
        try {
            ...
            delete input.file;
            let returnData = { id: product.id, ...input };
    ...
    async AllProducts(input) {
        try {
            ...
            if (input.hasOwnProperty('price') && (input.price.upperLimit != 0 || input.price.lowerLimit != 0)) {
            ...
            if (input.hasOwnProperty("discount")) {
                condition.discount = { [Op.gte]: input.discount }
            }
