...
export default class OrderService {
  ...
  async AddOrder(input) {
    try {
      ...
      if (!input.hasOwnProperty("deliveryCharge")) {
        input.deliveryCharge = 230;
      }
        ...
        await sequelize.transaction(async function (transaction) {
          for (const item of items) {
            let product = await models.Products.findOne(
              { where: { isDeleted: false, id: item.productId } },
              { transaction }
            );
          ...
          amount = amount + input.deliveryCharge;
    ...
    try {
      ...
      if (paymentIntent.status === "succeeded") {
        ...
        for (let i = 0; i < orderProduct.length; i++) {
          let product = await models.Products.findOne({
            where: { id: orderProduct[i].dataValues.productId },
          });
          ...
          product.update({
            quantity: product.quantity - orderProduct[i].dataValues.quantity,
          });
          await product.save();
        }
        ...
        let err = new Error();
        err.message = "cannot confirm order payment";
        err.name = "unprocessable";
        err.status = 422;
        throw err;
      }
